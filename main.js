...
function mtSelectMission(id){
  ...
  setTimeout(()=>{
    ...
    if(mtNarrationOn&&'speechSynthesis'in window){
      const utt=new SpeechSynthesisUtterance(mtMission.name+'. '+mtMission.tagline+'. '+(mtMission.phases[0]?.desc||'Launching from Earth.'));
      utt.rate=0.9;utt.pitch=0.85;utt.volume=0.9;
      const voices=window.speechSynthesis.getVoices();
      const pref=voices.find(v=>/google uk english male|daniel|reed/i.test(v.name))||voices.find(v=>v.lang==='en-GB')||voices.find(v=>v.lang.startsWith('en'));
      if(pref)utt.voice=pref;window.speechSynthesis.speak(utt);
    }
...
// Narration toggle
document.getElementById('mt-narr-btn')?.addEventListener('click',()=>{
  mtNarrationOn=!mtNarrationOn;
  const btn=document.getElementById('mt-narr-btn');
  btn.textContent = mtNarrationOn ? '🔊 NARRATE' : '🔇 MUTED';
  btn.classList.toggle('active', mtNarrationOn);
  if(!mtNarrationOn && window.speechSynthesis) window.speechSynthesis.cancel();
});
